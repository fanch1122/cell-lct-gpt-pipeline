...
$runtimePath = Join-Path (Split-Path $PSScriptRoot -Parent) 'runtime\cell_lct_cached_runtime.jsx'
...
if (-not $pythonCommand) { throw 'Python 3 was not found.' }
$prepareArguments = @($pythonPrefix + @(
    $prepareScript,
    '--input', $inputPath,
    '--output-dir', $workPath,
    '--job-id', $jobId,
...
$prepareOutput = & $pythonCommand.Source @prepareArguments
